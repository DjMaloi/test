...
async def improve_with_groq(original_answer: str, question: str) -> Optional[str]:
    ...
    """
    system_prompt = (
        "Ты помощник техподдержки. Переформулируй ответ так, чтобы он был понятен "
        "обычным пользователям без технических знаний.\n\n"
        "ПРАВИЛА:\n"
        "1. Сохраняй ВСЕ ссылки и технические идентификаторы как есть\n"
        "2. Не добавляй информацию, которой нет в оригинале\n"
        "3. Делай текст короче и проще, но не теряй смысл\n"
        "4. Максимум 800 символов\n"
        "5. Если непонятно — оставь как есть\n"
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ИСПРАВЛЕНО: В группах бот отвечает ВСЕМ, кроме админов из adminlist
    # (чтобы админы могли общаться между собой без вмешательства бота)
