...
def add_admin(user_id: int, username: str = None):
    adminlist[str(user_id)] = username or f"User_{user_id}"
    save_adminlist()
    logger.info(f"Пользователь {user_id} (@{adminlist[str(user_id)]}) добавлен в список администраторов")

def remove_admin(user_id: int):
    if str(user_id) in adminlist:
        del adminlist[str(user_id)]
        save_adminlist()
...
async def add_admin_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Если username не передан, пытаемся получить из Telegram
    if not username:
        try:
...
async def adminlist_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    admin_users = "\n".join([f"{uid}: @{name}" for uid, name in sorted(adminlist.items(), key=lambda x: int(x[0]))])
...
if __name__ == "__main__":
    ...
    logger.info("4.3 Бот с Админами группы и Админами бота, список админов с @username")
