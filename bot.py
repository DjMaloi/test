...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if source != "fallback" and len(best_answer) < 1000:
        prompt = f"""Используй текст полностью не сокращая и не удаляя ссылки в сообщении, текст должен быть локаничным и дружелюбным. Сохрани весь смысл. При выборе ответа всегда помни, что Киоск и Сканер это разные понятия. Если не нашел подоходящего ответа, уточни детали у пользователя.
Оригинал:
{best_answer}
